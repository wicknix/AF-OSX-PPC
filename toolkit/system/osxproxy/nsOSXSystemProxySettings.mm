...
IsHostProxyEntry(const nsACString& aHost, const nsACString& aOverride)
{
  ...
  nsAutoCString _override(aOverride);

  int32_t overrideLength = _override.Length();
  int32_t tokenStart = 0;
  int32_t offset = 0;
  bool star = false;

  while (tokenStart < overrideLength) {
    int32_t tokenEnd = _override.FindChar('*', tokenStart);
    if (tokenEnd == tokenStart) {
      ...
      if (_override.FindChar('.', tokenStart) == tokenStart)
        tokenStart++;
    } else {
      if (tokenEnd == -1)
        tokenEnd = overrideLength; // no '*' char, match rest of string
      nsAutoCString token(Substring(_override, tokenStart, tokenEnd - tokenStart));
