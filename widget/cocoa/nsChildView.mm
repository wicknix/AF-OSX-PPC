...
nsChildView::NotifyIMEInternal(const IMENotification& aIMENotification)
{
  ...
  switch (aIMENotification.mMessage) {
    ...
    case NOTIFY_IME_OF_BLUR:
      ...
      return NS_OK;
    case NOTIFY_IME_OF_SELECTION_CHANGE:
      // XXX mTextInputHandler->OnSelectionChange();
      return NS_OK;
    default:
      return NS_ERROR_NOT_IMPLEMENTED;
  }
...
static void MaybeDrawResizeIndicatorCallback(gfx::DrawTarget* drawTarget,
	const nsIntRegion& updateRegion) {
...
#else
  mResizerImage->UpdateIfNeeded(size, nsIntRegion(),
...
#else
  mCornerMaskImage->UpdateIfNeeded(size, nsIntRegion(),
...
- (CGSRegionObj)_regionForOpaqueDescendants:(NSRect)aRect forMove:(BOOL)aForMove
{
  ...
  nsTArray<CGRect> rects;
  nsIntRegionRectIterator iter(opaqueRegion);
  for (;;) {
    const nsIntRect* r = iter.Next();
    if (!r)
      break;
...
*/
  NSPoint locationInWindow = [[self window]
...
#else
  NSUInteger mouseButtons = 0; // wtf, but this works.
#endif
...
- (void)sendCompositionEvent:(int32_t) aEventType
{
  ...
  if (aEventType == NS_COMPOSITION_END)
                      ...
                      doCommit:(BOOL) doCommit
{
  ...
  WidgetCompositionEvent textEvent(true, NS_COMPOSITION_CHANGE, mGeckoChild);
...
#define MAX_BUFFER_SIZE 32
  ...
  if (len == 1 && !nsTSMManager::IsComposing()) {
    ...
    WidgetKeyboardEvent geckoEvent(true, NS_KEY_PRESS, mGeckoChild);
...
- (void) setMarkedText:(id)aString selectedRange:(NSRange)selRange
{
  ...
  if (!nsTSMManager::IsComposing() && len > 0) {
	...
	selection(true, NS_QUERY_SELECTED_TEXT, mGeckoChild);
...
- (NSInteger) conversationIdentifier
{
  ...
  if (!mGeckoChild)
    return (long)self;
  WidgetQueryContentEvent textContent(true, NS_QUERY_TEXT_CONTENT, mGeckoChild);
  textContent.InitForQueryTextContent(0, 0);
...
- (NSRange) selectedRange
{
  ...
  if (!mGeckoChild)
    return NSMakeRange(NSNotFound, 0);
  WidgetQueryContentEvent selection(true, NS_QUERY_SELECTED_TEXT, mGeckoChild);
  mGeckoChild->DispatchWindowEvent(selection);
...
- (NSRect) firstRectForCharacterRange:(NSRange)theRange
{
  ...
  if (!useCaretRect) {
    WidgetQueryContentEvent charRect(true, NS_QUERY_TEXT_RECT, mGeckoChild);
  ...
  if (useCaretRect) {
    WidgetQueryContentEvent caretRect(true, NS_QUERY_CARET_RECT, mGeckoChild);
...
// It's 10.5+, so we don't implement it. See bug 875674.
...
((nsTSMManager::GetIMEOpenState()) ? "TRUE" : "false")));
  ...
  if (nonDeadKeyPress && !nsTSMManager::IsComposing()) {
    ...
    WidgetKeyboardEvent geckoKeydown(true, NS_KEY_DOWN, mGeckoChild);
    ...
    if (modifierFlags == NSControlKeyMask && [[theEvent charactersIgnoringModifiers] isEqualToString:@" "]) {
      WidgetMouseEvent contextMenuEvent(true, NS_CONTEXTMENU, [self widget], WidgetMouseEvent::eReal, WidgetMouseEvent::eContextMenuKey);
    ...
    WidgetKeyboardEvent geckoKeypress(true, NS_KEY_PRESS, mGeckoChild);
...
- (void)initTSMDocument
{
  ...
  WidgetQueryContentEvent textContent(true, NS_QUERY_TEXT_CONTENT,
                                  mGeckoChild);
  textContent.InitForQueryTextContent(0, 0);
  mGeckoChild->DispatchWindowEvent(textContent);
...
- (void)keyUp:(NSEvent*)theEvent
{
  ...
  if (!nsCocoaFeatures::OnLeopardOrLater() && !keyUpAlreadySentKeyDown &&
    ...
    if (![nativeKeyDownEvent isARepeat]) {
      WidgetKeyboardEvent geckoEvent(true, NS_KEY_DOWN, mGeckoChild);
  ...
  WidgetKeyboardEvent geckoEvent(true, NS_KEY_UP, mGeckoChild);
