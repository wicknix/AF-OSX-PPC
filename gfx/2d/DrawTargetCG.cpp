  ...
  if (aPattern.GetType() == PatternType::LINEAR_GRADIENT) {
    ...
    if (stops->mExtend == ExtendMode::CLAMP) {
      ...
      CGContextDrawRadialGradient(cg, stops->mGradient, startCenter, startRadius, endCenter, endRadius,
                                  kCGGradientDrawsBeforeStartLocation | kCGGradientDrawsAfterEndLocation);
#else
	  stops->DrawTigerRadialGradient(cg, startCenter, startRadius,
		endCenter, endRadius);
#endif
    } else if (stops->mExtend == ExtendMode::REPEAT || stops->mExtend == ExtendMode::REFLECT) {
                         ...
                         const GlyphRenderingOptions *aGlyphRenderingOptions)
{
  ...
  if (!isGradient(aPattern))  {
    	CGContextSetTextDrawingMode(cg, kCGTextFill);
    	SetFillFromPattern(cg, mColorSpace, aPattern);
    // XXX: Profile this and find out how much is in a typical run.
    // Perhaps the loop can be unrolled in typical cases.
 	for (unsigned int i = 0; i < aBuffer.mNumGlyphs; i++) {
...
#else
  // 10.4 does not support IOSurface.
  return (aType == NativeSurfaceType::CGCONTEXT) ? mCg : nullptr ;
#endif
}
