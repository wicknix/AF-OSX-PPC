...
class CodeGeneratorPPC : public CodeGeneratorShared
{
  ...
  protected:
    ...
    void bailoutCmp32(Assembler::Condition c, T1 lhs, T2 rhs, LSnapshot *snapshot) {
        ...
        // XXX. This is rather branchy in the default case and it would be nice
        // to pull the bailouts out of line.
        masm.bc(masm.ma_cmp(lhs, rhs, Assembler::InvertCondition(c)), &skip);
        bailout(snapshot);
        masm.bind(&skip);
    }
    template<typename T>
