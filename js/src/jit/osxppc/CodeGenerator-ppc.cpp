...
/*
    masm.bc(PPCRegisters::cr1, PPCAssembler::ConditionGE, 4*4,
...
*/
    ...
    // zero.
/*
    masm.bc(PPCRegisters::cr7, PPCAssembler::ConditionGT, 23*4,
...
*/
    ...
    // If set, return src.
/*
    masm.bc(PPCRegisters::cr7, PPCAssembler::ConditionEQ, 20*4,
            PPCAssembler::NotLikelyBranch,
...
CodeGeneratorPPC::visitDivI(LDivI *ins)
{
	...
	// we must bail out. However, only do this if there is a snapshot, because
	// if there isn't, then Ion already knows we can't (mustn't) fail.
	if (ins->snapshot()) {
		masm.bc(Assembler::Overflow, &overflowException);
		bailoutFrom(&overflowException, ins->snapshot());
	}
		
	// If we cannot truncate the result, compute the remainder, and bailout
	// if there is one.
    if (!mir->canTruncateRemainder()) {
        ...
        masm.subf(temp, tempRegister, lhs);
        masm.and__rc(tempRegister, temp, temp);
        masm.bc(Assembler::NonZero, &remainderNonZero);
...
CodeGeneratorPPC::visitModI(LModI *ins)
{
    ...
    if (mir->canBeNegativeDividend()) {
        ...
        if (mir->isTruncated()) {
            // (INT_MIN % -1)|0 == 0
            BufferOffset skip = masm._bc(0,
                masm.ma_cmp(lhs, Imm32(-1), Assembler::NotEqual), cr0,
                Assembler::LikelyB);
    ...
    if (mir->canBeNegativeDividend()) {
        ...
        masm.bc(masm.ma_cmp(rhs, Imm32(0), Assembler::GreaterThan), &notNegative); // SHORT
        if (mir->isTruncated()) {
            // NaN|0 == 0 and (0 % -X)|0 == 0
            BufferOffset skip = masm._bc(0,
...
CodeGeneratorPPC::visitOutOfLineTableSwitch(OutOfLineTableSwitch *ool)
{
    ...
    for (size_t i = 0; i < mir->numCases(); i++) {
		...
		masm.ma_li32(tempRegister, cl.dest()); // Always patchable, always two words.
                                           ...
                                           Register address)
{
    ...
    masm.ma_li32(address, ool->jumpLabel()->dest());
...
// Common code for visitMathD/visitMathF.
static void
JSOpToFloat(JSOp what, MacroAssembler &masm, const LAllocation *src1, const LAllocation *src2,
...
CodeGeneratorPPC::visitMathD(LMathD *math)
{
    ...
    JSOpToFloat(math->jsop(), masm, src1, src2, output);
}
...
CodeGeneratorPPC::visitMathF(LMathF *math)
{
    ...
    const LDefinition *output = math->getDef(0);
    JSOpToFloat(math->jsop(), masm, src1, src2, output);
    // Because Ion may expect to use this value right away, we do the rounding step
    // right here. Baseline doesn't need this because it always calls
    // convertDoubleToFloat32 before it does any loads, stores or calls.
    // We also do this in visitSqrtF.
    //
    // TODO: We probably need to implement the single precision instructions
    // for no other reason than to reduce I-cache pressure with lots of back-to-back
    // FPU operations.
    masm.frsp(ToFloatRegister(output), ToFloatRegister(output));
}
...
{
    // Can always use r12, for most operations.
...
void
CodeGeneratorPPC::visitUDiv(LUDiv *ins)
{
	ADBlock("visitUDiv");
...
void
CodeGeneratorPPC::visitUMod(LUMod *ins)
{
	ADBlock("visitUMod");
