                                              ...
                                              nsACString& aOverride)
{
  ...
  nsAdoptingCString _override = mozilla::Preferences::GetCString(PromiseFlatCString(pref).get());
  if (_override) {
    aOverride = _override;
  }
  else {
                  ...
                  void* aArg)
{
  ...
  OverrideMapping _override = {
    chromeURI, overrideURI
  };
  overrides->AppendElement(_override);
                                         ...
                                         char *const * argv, int flags)
{
  ...
  if (mDynamicRegistration) {
    ...
    OverrideMapping _override = { serializedChrome, serializedOverride };
    SendManifestEntry(_override);
  }
}
